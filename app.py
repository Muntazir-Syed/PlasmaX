...
steps    = st.sidebar.slider("Steps per Cycle",     50, 400, 200)
...
# Persistent stop flag across reruns
...
if start:
    ...
    # Unique session prefix — prevents run_000…run_049 from a second session
    # overwriting files from the first session.
    ...
    for i in range(n_cycles):
        ...
        run_id     = f"{session_ts}_cycle_{i:03d}"
        ...
        m_temp.metric(
            "🌡 Temperature (keV)",
            f"{t_now:.1f}",
            delta=_delta(prev, "temperature", t_now),
            delta_color="inverse",
        )
        m_pres.metric(
            "💨 Pressure (atm)",
        ...
        # ---- Chart: all three signals on dual-axis subplot ----
        fig = make_subplots(
            rows=2, cols=1,
            shared_xaxes=True,
            row_heights=[0.6, 0.4],
            subplot_titles=("Temperature & Pressure", "Magnetic Field"),
...
experiments = load_experiments(limit=10, include_data=False)
...
if experiments:
    ...
    for exp in reversed(experiments[-10:]):
        ...
        with st.expander(
            ...
            c1.metric("Temp Max",    f"{summary.get('temp_max',  '—'):.1f}" if summary else "—")
            c1.metric("Temp Mean",   f"{summary.get('temp_mean', '—'):.1f}" if summary else "—")
            c2.metric("Pres Max",    f"{summary.get('pres_max',  '—'):.2f}" if summary else "—")
            c2.metric("Pres Mean",   f"{summary.get('pres_mean', '—'):.2f}" if summary else "—")
            c3.metric("Mag Min",     f"{summary.get('mag_min',   '—'):.3f}" if summary else "—")
            c3.metric("Mag Mean",    f"{summary.get('mag_mean',  '—'):.3f}" if summary else "—")
            st.info(
